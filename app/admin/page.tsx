...
export default function AdminPage() {
  const [tab, setTab] = useState<'audience' | 'unmatched'>('unmatched')
  const [audienceReviews, setAudienceReviews] = useState<any[]>([])
  const [unmatched, setUnmatched] = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  // Form state for creating a show from unmatched
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '32px 24px', flex: 1, width: '100%' }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid #1e1e2e', marginBottom: '24px' }}>
          <button style={tabStyle('unmatched')} onClick={() => setTab('unmatched')}>
            Unmatched reviews {unmatched.length > 0 && <span style={{ marginLeft: '6px', background: '#1D9E75', color: 'white', fontSize: '11px', padding: '1px 6px', borderRadius: '10px' }}>{unmatched.length}</span>}
          </button>
          <button style={tabStyle('audience')} onClick={() => setTab('audience')}>
            Audience queue {audienceReviews.length > 0 && <span style={{ marginLeft: '6px', background: '#4b5563', color: 'white', fontSize: '11px', padding: '1px 6px', borderRadius: '10px' }}>{audienceReviews.length}</span>}
          </button>
        </div>

        {loading && <p style={{ color: '#4b5563' }}>Loading...</p>}

        {/* Unmatched reviews tab */}
        {!loading && tab === 'unmatched' && (
          <div>
            {unmatched.length === 0 && <p style={{ color: '#4b5563', fontSize: '14px' }}>No unmatched reviews — pipeline is matching everything.</p>}
              ...
              {unmatched.map(item => (
                <div key={item.id} style={{ background: '#1e1e2e', border: '1px solid #2a2a3e', borderRadius: '10px', padding: '18px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '8px' }}>
                    <div>
                      <div style={{ fontFamily: 'Georgia, serif', fontSize: '16px', fontWeight: '600', color: '#f1f5f9', marginBottom: '3px' }}>{item.show_title || 'Unknown title'}</div>
                      <div style={{ fontSize: '12px', color: '#6b7280' }}>
                        {item.outlet} {item.reviewer && `· ${item.reviewer}`} · {item.city} · {item.published_date}
                  ...
                  {item.pull_quote && (
                    <p style={{ fontSize: '13px', color: '#9ca3af', fontStyle: 'italic', margin: '0 0 10px 0', lineHeight: '1.5' }}>"{item.pull_quote}"</p>
                  )}

                  {item.source_url && (
                    <a href={item.source_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '12px', color: '#1D9E75', textDecoration: 'none', display: 'block', marginBottom: '12px' }}>
                      Read review →
                    </a>
                  )}
                  ...
                  {creating === item.id ? (
                    <div style={{ background: '#14141f', border: '1px solid #2a2a3e', borderRadius: '8px', padding: '16px', marginTop: '8px' }}>
                      <p style={{ fontSize: '12px', fontWeight: '600', color: '#9ca3af', margin: '0 0 12px 0', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Create show & production</p>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button onClick={() => createAndApprove(item)} disabled={saving} style={{ flex: 1, background: '#1D9E75', color: 'white', border: 'none', borderRadius: '6px', padding: '9px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>
                          {saving ? 'Saving...' : '✓ Create show & approve review'}
                        </button>
                        <button onClick={() => setCreating(null)} style={{ background: 'none', border: '1px solid #2a2a3e', color: '#6b7280', borderRadius: '6px', padding: '9px 14px', fontSize: '13px', cursor: 'pointer' }}>
                          Cancel
                        </button>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button onClick={() => startCreate(item)} style={{ flex: 1, background: '#1D9E75', color: 'white', border: 'none', borderRadius: '6px', padding: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
                        + Create show & approve
                      </button>
                      <button onClick={() => dismissUnmatched(item.id)} style={{ background: 'none', border: '1px solid #2a2a3e', color: '#6b7280', borderRadius: '6px', padding: '8px 14px', fontSize: '13px', cursor: 'pointer' }}>
                        Dismiss
                      </button>
                    </div>
                  )}
              ...
              {audienceReviews.map(review => (
                <div key={review.id} style={{ background: '#1e1e2e', border: '1px solid #2a2a3e', borderRadius: '10px', padding: '18px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <span style={{ fontFamily: 'Georgia, serif', fontSize: '15px', color: '#f1f5f9' }}>{(review.productions as any)?.shows?.title}</span>
                    <span style={{ color: '#1D9E75' }}>{'★'.repeat(review.star_rating)}</span>
                  </div>
                  {review.review_text && <p style={{ fontSize: '13px', color: '#9ca3af', fontStyle: 'italic', margin: '0 0 12px 0' }}>"{review.review_text}"</p>}
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={() => updateAudience(review.id, 'approved')} style={{ flex: 1, background: '#1D9E75', color: 'white', border: 'none', borderRadius: '6px', padding: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>✓ Approve</button>
                    <button onClick={() => updateAudience(review.id, 'rejected')} style={{ flex: 1, background: 'none', border: '1px solid #2a2a3e', color: '#6b7280', borderRadius: '6px', padding: '8px', fontSize: '13px', cursor: 'pointer' }}>✕ Reject</button>
                  </div>
                </div>
              ))}
            </div>
